...
public:
	...
	Vector operator + (Vector&);				// like a function,  define a operator on this class, to  sum vector as C = A + B ;
...
int main(int argc, const char * argv[])
{
	...
	A.add(locA+1, 4); // should be same size, therefore we are adding locA+1
	B.add(locA+1, 4); // should be same size, therefore we are adding locA+1
A.ListAll();
    A.ListNonZero(); //cout << A.getSize() << " items found..." << endl;
	B.ListNonZero(); //cout << B.getSize() << " items found..." << endl;
	SumVector = A + B;
